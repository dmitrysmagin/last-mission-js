...
// Game keys array (mirrors original C GKeys[7])
export const GKeys = new Uint8Array(7);

// System keys array (mirrors original C Keys[128])
export const Keys = new Uint8Array(128);

// DOM key to scancode mapping (mirrors original input.c)
const keyToScan = {
  'ArrowUp': SC_UP,
  'ArrowDown': SC_DOWN,
  'ArrowLeft': SC_LEFT,
...
export function input_anykey() {
  return Keys.some(v => v === 1) ? 1 : 0;
}

export function input_poll() {
  // Keys are already updated by event listeners.
  // This mirrors SDL_PollEvent being called each frame.
}

...
export let ctx: CanvasRenderingContext2D = backbuffer.getContext('2d')!;

export function setRenderTarget(canvas?: HTMLCanvasElement): void {
...
export function gfx_init(): boolean {
  resizeDisplay();
  return true;
}

export function gfx_quit(): void {
}

export function gfx_flip(): void {
...
export let spriteBuffer: Uint8Array | null = null;
export let tileBuffer: Uint8Array | null = null;
export let palette: Uint8Array | null = null;
...
function parseIndexedBMP(buffer: ArrayBuffer): ParsedBMP {
  ...
  for (let i = 0; i < 256; i++) {
    const off = 54 + i * 4;
    pal[i * 3]     = dv.getUint8(off + 2);
    pal[i * 3 + 1] = dv.getUint8(off + 1);
    pal[i * 3 + 2] = dv.getUint8(off);
  }
...
export let logoCanvas: HTMLCanvasElement | null = null;
...
export const splashData: SplashImage[] = [];
...
export function DrawSplash(splashIdx: number): void {

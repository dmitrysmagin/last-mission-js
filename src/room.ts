...
const ROOM_WIDTH = (SCREEN_WIDTH / 8) | 0;
const ROOM_HEIGHT = (ACTION_SCREEN_HEIGHT / 8) | 0;

const screen = new Uint16Array(ROOM_WIDTH * ROOM_HEIGHT);

export function GetTileI(x: number, y: number): number {
  if (x < 0 || x >= ROOM_WIDTH || y < 0 || y >= ROOM_HEIGHT)
    return 0;
  return screen[y * ROOM_WIDTH + x];
}

export function SetTileI(x: number, y: number, value: number): void {
  if (x < 0 || x >= ROOM_WIDTH || y < 0 || y >= ROOM_HEIGHT)
    return;
  screen[y * ROOM_WIDTH + x] = value;
}

export function UnpackRoom(world: World, roomIndex: number): void {
  screen.fill(0);
...
export function BlitRoomOutlines(world: World, roomIndex: number): void {
  const room = world.room[roomIndex];
...
export function BlitBackground(world: World, roomIndex: number): void {
...
export function rgb565ToCSS(color: number): string {
